...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((o) => {
      return o
        .setName("username")
        .setDescription("The username of the your Minecraft account")
        .setRequired(true);
    }),
  exec: async (i: CommandInteraction, c: HyviewClient, h: Client) => {
    ...
    if (e) {
      i.reply({
        embeds: [
          c.embed({
            desc: new MessageEmitter().link.DB_RECORD_EXISTS(),
            type: "INFO",
          }),
        ],
      });
      ...
      await HyviewModels.User.create({ discord_id: i.user.id }).then((d) => {
        i.reply({
          embeds: [
            c.embed({
              desc: new MessageEmitter().link.DB_RECORD_NOT_EXISTS_SO_CREATED(),
              type: "INFO",
            }),
          ],
        });
    ...
    async function linkAccount(i: CommandInteraction) {
      ...
      try {
        ...
        isLinked = await h
          .getPlayer(i.options.getString("username", true))
          .then((player) => {
            var d: any[] = [];
        ...
        switch (e.message) {
          case Hypixel.Errors.PLAYER_DOES_NOT_EXIST:
            i.followUp({
              embeds: [
                c.embed({
                  desc: new MessageEmitter().link.PLAYER_DOES_NOT_EXIST(),
                  type: "WARNING",
                }),
              ],
            });
          ...
          case Hypixel.Errors.PLAYER_HAS_NEVER_LOGGED:
            i.followUp({
              embeds: [
                c.embed({
                  desc: new MessageEmitter().link.PLAYER_HAS_NOT_LOGGED(),
                  type: "WARNING",
                }),
              ],
            });
          ...
          default:
            i.followUp({
              embeds: [
                c.embed({
                  desc: new MessageEmitter().GENERIC_ERROR(e),
                  type: "DANGER",
                }),
              ],
            });
      ...
      if (r && r.get("uuid")) {
        i.editReply({
          embeds: [
            c.embed({
              desc: new MessageEmitter().link.ALREADY_LINKED(),
              type: "INFO",
            }),
          ],
        });
        ...
        if (isLinked.length === 0) {
          i.replied
            ? i.editReply({
                embeds: [
                  c.embed({
                    desc: new MessageEmitter().link.ACCOUNT_NOT_LINKED_ON_HYPIXEL(),
                    type: "WARNING",
                  }),
                ],
              })
          ...
          if (isLinked[0].link !== i.user.tag) {
            i.replied
              ? i.editReply({
                  embeds: [
                    c.embed({
                      desc: new MessageEmitter().link.TAG_DOES_NOT_MATCH(
                        isLinked[0].link
                      ),
                      type: "INFO",
                    }),
            ...
            i.replied
              ? i.editReply({
                  embeds: [
                    c.embed({
                      desc: new MessageEmitter().link.DISCORD_ACCOUNT_FOUND(
                        isLinked[0].link
                      ),
                      type: "SUCCESS",
                    }),
            ...
            if (!r) {
              i.followUp({
                embeds: [
                  c.embed({
                    desc: 'An unexpected error has occured. Please report it to the development team, and tell them the following:\n`The "commands/link.ts" file returned null when fetching a Hypixel player in line 35. Please correct this.`',
                  }),
                ],
              });
              return;
