...
export default class HyviewClient extends Client {
  ...
  public embed(e: ClientEmbedOptions): MessageEmbed {
    const em = new MessageEmbed({
      description: e.desc,
      fields: e.main || undefined,
      color: e.color ? e.color : this.validateEmbedType(e.type!),
      title: e.name || undefined,
      footer: { text: e.footer || undefined },
    });

    if (e.img !== undefined) {
      em.setThumbnail(e.img);
    }
  ...
  public fetchRankProps(r: string | undefined): RankProps | undefined {
    ...
    switch (r) {
      case "VIP":
        d = {
          rank: "VIP",
          rankColor: this._mccolorcodes.GREEN as ColorResolvable,
          emoji: this._emojis.VIPIcon,
        };
        break;
      case "VIP+":
        d = {
          rank: "VIP+",
          rankColor: this._mccolorcodes.GREEN as ColorResolvable,
          emoji: this._emojis.VIPPlusIcon,
        };
        break;
      case "MVP":
        d = {
          rank: "MVP",
          rankColor: this._mccolorcodes.AQUA as ColorResolvable,
          emoji: this._emojis.MVPIcon,
        };
        break;
      case "MVP+":
        d = {
          rank: "MVP+",
          rankColor: this._mccolorcodes.AQUA as ColorResolvable,
          emoji: this._emojis.MVPPlusIcon,
        };
        break;
      case "MVP++":
        d = {
          rank: "MVP++",
          rankColor: this._mccolorcodes.GOLD as ColorResolvable,
          emoji: this._emojis.MVPPlusPlusIcon,
        };
        break;
      case "PIG+++":
        d = {
      ...
      case "Game Master":
        d = {
          rank: "Game Master",
          rankColor: this._mccolorcodes.DARK_GREEN as ColorResolvable,
          emoji: this._emojis.HypixelStaff,
        };
        break;
      case "Admin":
        d = {
          rank: "Admin",
          rankColor: this._mccolorcodes.RED as ColorResolvable,
          emoji: this._emojis.HypixelStaff,
        };
        break;
      case "OWNER":
      ...
      default:
        d = {
          rank: "Default",
          rankColor: this._mccolorcodes.LIGHT_GREY as ColorResolvable,
          emoji: "",
        };
        break;
    }
...
class HyviewChannelLogger {
  ...
  public async userJoin(u: User, g: Guild): Promise<void> {
    (this.client.channels.cache.get(this.channel) as TextChannel).send({
      embeds: [
        this.client.embed({
          desc: `${u.tag} joined the server. We're now at ${
            g.memberCount - 3
          } members.`,
          type: "SUCCESS",
        }),
  ...
  public async userLeave(u: User, g: Guild): Promise<void> {
    (this.client.channels.cache.get(this.channel) as TextChannel).send({
      embeds: [
        this.client.embed({
          desc: `${u.tag} left the server. We're now at ${
            g.memberCount - 3
          } members.`,
          type: "DANGER",
        }),
  ...
  public async userVerify(u: User, t: Number): Promise<void> {
    (this.client.channels.cache.get(this.channel) as TextChannel).send({
      embeds: [
        this.client.embed({
          desc: `${u.tag} successfully verified after ${t} tries.`,
          type: "INFO",
        }),
      ],
    });
  ...
  public async userNicknameChange(
    u: User,
    b: string,
    a: string
  ): Promise<void> {
    (this.client.channels.cache.get(this.channel) as TextChannel).send({
      embeds: [
        this.client.embed({
          desc: `${u.tag} changed their nickname.\n\n **Before:** ${b}\n**After:** ${a}`,
          type: "INFO",
        }),
      ],
    });
  ...
  public async userRoleRemove(u: User, r: string): Promise<void> {
    (this.client.channels.cache.get(this.channel) as TextChannel).send({
      embeds: [
        this.client.embed({
          desc: `${u.tag} was removed from **${r}** role.`,
          type: "DANGER",
        }),
      ],
    });
