...
export default class MessageEmitter {
    ...
    STAFF_ONLY(): string {
        return Emojis.default.ShieldInfo + " You need to be a Hyview staff member to use this command!"
    }

    SENIOR_STAFF_ONLY(): string {
        return Emojis.default.ShieldInfo + " You need to be a Hyview senior staff member to use this command!"
    }
...
class LinkCommandMessageEmitter {
    ...
    DB_RECORD_EXISTS(): string {
        return Emojis.default.TickInfo + " We found an existing record for your Discord account in our database, now attempting to link your Minecraft account.";
    }

    DB_RECORD_NOT_EXISTS_SO_CREATED(): string {
        return Emojis.default.TickInfo + " You weren't in our database so we've created a record for your Discord account, now attempting to link your Minecraft account.";
    }

    PLAYER_DOES_NOT_EXIST(): string {
        return Emojis.default.ExclamationWarning + " Looks like that Minecraft player doesn't exist!";
    }

    PLAYER_HAS_NOT_LOGGED(): string {
        return Emojis.default.ExclamationWarning + " Looks like that Minecraft player has never logged into Hypixel!";
    }

    ACCOUNT_NOT_LINKED_ON_HYPIXEL(): string {
        return Emojis.default.QuestionWarning + " Looks like you don't have a Discord account linked on Hypixel! Log on and add it to your social media connections.";
    }

    TAG_DOES_NOT_MATCH(l: string): string {
        return Emojis.default.TickInfo + " We found a Discord account on your Hypixel profile (" + l + "), but it doesn't match your current tag. If you changed your tag since you connected your Discord account to Hypixel, you will need to log in to Hypixel, un-register your account and then re-register it to reset the issue."
    }

    ALREADY_LINKED(): string {
        return Emojis.default.TickInfo + " You already have a linked Minecraft account in our database! Contact the Hyview staff team if this is in error."
    }

    DISCORD_ACCOUNT_FOUND(t: string): string {
        return Emojis.default.TickSuccess + " We found this Discord account on your Hypixel profile: " + t;
    }
...
class ProfileCommandMessageEmitter {
    ...
    MC_USERNAME_DEPWARN(): string {
        return Emojis.default.ShieldWarning + " Using a Minecraft username for /profile is not recommended as the user's account may not be linked."
    }
...
class BetaProgramMessageEmitter {
    ...
    ALREADY_ADDED_USER(): string {
        return Emojis.default.CrossDanger + " That user has already been added to the program!"
    }
